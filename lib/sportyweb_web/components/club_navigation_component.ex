defmodule SportywebWeb.ClubNavigationComponent do
    ...
    <nav class="sticky top-10">
      <div class="space-y-1">
        ...
        <.link
          ...
          phx-value-item={:contacts_menue}
          class={[
            @classes_menu_item,
            if(@club_navigation_current_item == :contacts_menue, do: @classes_menu_item_active)
          ]}
        >
        ...
        <ul class={["mb-1 px-2", if(!@show_submenu_contacts, do: "hidden")]}>
          ...
          <li>
            <.link
              navigate={~p"/clubs/#{@club}/contact_groups"}
              class={[
                @classes_menu_item,
                @classes_submenu_item,
                if(@club_navigation_current_item == :contact_groups, do: @classes_menu_item_active)
              ]}
            >
              <span class="truncate">Kontaktgruppen</span>
            </.link>
          </li>
          ...
          <li>
            <.link
              navigate={~p"/clubs/#{@club}/members/inventory_list"}
              class={[
                @classes_menu_item,
                @classes_submenu_item,
                if(@club_navigation_current_item == :members_inventory, do: @classes_menu_item_active)
              ]}
            >
              <span class="truncate">Mitglieder Bestandserhebung</span>
            </.link>
          </li>
          <li>
            <.link
              navigate={~p"/clubs/#{@club}/members/qualifications"}
              class={[
                @classes_menu_item,
                @classes_submenu_item,
                if(@club_navigation_current_item == :members_qualifications,
                  do: @classes_menu_item_active
                )
              ]}
            >
              <span class="truncate">Mitgliederqualifikationen</span>
            </.link>
          </li>
        </ul>
        <.link
  ...
  def update(assigns, socket) do
    ...
    show_submenu_contacts =
      assigns.club_navigation_current_item == :contact_search ||
        ...
        assigns.club_navigation_current_item == :members_inventory ||
        assigns.club_navigation_current_item == :members_qualifications
  ...
  @impl true
  def handle_event("toggle_submenu", %{"item" => "contacts_menue"}, socket) do
