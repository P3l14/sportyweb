defmodule Sportyweb.Personal do
  ...
  def search(%{"club_id" => club_id, "type" => type} = search_params) do
    ...
    |> Enum.reduce(
      ...
      fn
        ...
        {"organization_type", organization_type}, query ->
          query |> where([c], c.organization_type == ^organization_type)

        {"person_last_name", person_last_name}, query ->
          query |> where([c], like(c.person_last_name, ^person_last_name))

        {"person_birth_name", person_birth_name}, query ->
          query |> where([c], like(c.person_birth_name, ^person_birth_name))

        {"person_middle_names", person_middle_names}, query ->
          query |> where([c], like(c.person_middle_names, ^person_middle_names))

        {"person_first_name", person_first_name}, query ->
          query |> where([c], like(c.person_first_name, ^person_first_name))

        {"person_birthday", person_birthday}, query ->
          query |> where([c], c.person_birthday == ^person_birthday)

        {"person_gender", person_gender}, query ->
  ...
  def search2(%{"club_id" => club_id, "type" => type} = search_params) do
    query = contact_base_query(club_id, type)
